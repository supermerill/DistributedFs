...
public class PanelParameterPeer extends InstallPanel {
	...
	JTextField txtInstallPath = new JTextField();
	...
	public PanelParameterPeer() {
		...
		btNext.addActionListener((actEvent)->{
			...
			if(!new File(txtInstallPath.getText()).exists()) {
				JOptionPane.showMessageDialog(this, "Error: Failed to create directory '"+txtInstallPath.getText()+"'", "Error", JOptionPane.WARNING_MESSAGE);
				return;
			}
			manager.finish();
		});
